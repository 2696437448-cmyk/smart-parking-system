...
def main() -> None:
    ...
    assert_contains(
        ...
        ["--color-bg-main", "--color-surface-card", "--color-brand-primary", "--color-panel-glow", "--font-display"],
    )
    assert_contains(
        FRONTEND / "src" / "styles" / "base.css",
        ["radial-gradient", ".tech-theme-root", ".app-grid-glow", "backdrop-filter"],
    )
    assert_contains(FRONTEND / "src" / "layouts" / "OwnerLayout.vue", ["a-tag", "owner-sidebar", "首页"])
    assert_contains(FRONTEND / "src" / "layouts" / "AdminLayout.vue", ["a-tag", "admin-header", "物业监管"])
    assert_contains(FRONTEND / "src" / "styles" / "layout.css", [".owner-sidebar", ".admin-header", ".shell-status-strip"])
    ...
    assert_contains(FRONTEND / "src" / "pages" / "OwnerDashboard.vue", ["a-form", "a-button", "dashboard-grid"])
    assert_contains(FRONTEND / "src" / "pages" / "OwnerOrders.vue", ["orders-grid", "a-button", "ViewStateNotice"])
    assert_contains(FRONTEND / "src" / "pages" / "OwnerNavigation.vue", ["navigation-grid", "a-tag", "MapPreview"])
    assert_contains(FRONTEND / "src" / "styles" / "pages.css", [".dashboard-grid", ".orders-grid", ".navigation-grid"])
    assert_contains(FRONTEND / "src" / "pages" / "AdminMonitor.vue", ["admin-summary-board", "a-button", "a-tag", "EChartPanel"])
    assert_contains(FRONTEND / "src" / "styles" / "pages.css", [".admin-summary-board", ".admin-main-chart", ".admin-side-cards"])
