...
- `OwnerDashboard.vue`
  页面标题可以理解成“智能预约中枢”，它是业主端的推荐入口页
- `OwnerOrders.vue`
  页面标题可以理解成“订单与账单任务流”，它承接预约后的账单和结算
- `OwnerNavigation.vue`
  页面标题可以理解成“目标车位导航”，它承接同一个 `orderId` 的到达阶段
- `AdminMonitor.vue`
  页面标题可以理解成“停车运营驾驶舱”，它不是普通监控页，而是经营视图总入口
...
3. `OwnerLayout.vue` 提供业主端 command bar 和状态标签
...
这里的“科技感”不是简单换个颜色，而是三件事叠加出来的：

- 统一的深色主题与状态标签
- owner / admin 两套明确的页面壳层
- 摘要卡片、标题卡片、图表卡片、地图卡片的统一外观
