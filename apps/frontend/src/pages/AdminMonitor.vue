...
<template>
  <section class="page-grid admin-page-grid admin-dashboard operations-cockpit">
    <section class="operations-primary-grid">
      <article class="panel hero-card admin-hero primary-kpi-panel" v-motion-slide-visible-once-bottom>
      ...
      <div class="primary-chart-card" v-if="dashboard" v-motion-slide-visible-once-top>
    ...
    <section class="operations-secondary-grid">
      <article class="panel summary-panel kpi-signal-grid" v-motion-slide-visible-once-top>
        <SectionHeader
          ...
          title="经营亮点摘要"
          subtitle="先看区域、收益采样和预测点位，再决定是否进入细节图表。"
      ...
      <article class="panel insight-panel" v-motion-slide-visible-once-top>
        <SectionHeader
          eyebrow="Runtime Narrative"
          title="运行解释层"
          subtitle="让页面说明当前数据来自哪里、什么时候更新、是否处于降级运行。"
          :badge="adminSummary.dispatchBadge"
        />
      ...
      <div class="chart-cluster secondary-chart-cluster" v-if="dashboard" v-motion-slide-visible-once-bottom>
