...
1. 当前默认演示基线为 `Step40`。
2. 含义：主链业务闭环、近真实数据接入、双端业务页面、地图导航增强、物业端一屏监管、发布化基线、dashboard 合同收敛、view-model 下沉、聚合层模块化与综合验收均已通过。
3. 历史基线：
   - `Step36`：发布化稳定锚点
   - `Step30`：功能与增强交付基线
   - `Step24`：主链基线
...
4. 登录后业主端：`http://localhost:4173/owner/dashboard`
5. 业主端订单：`http://localhost:4173/owner/orders`
6. 业主端导航：`http://localhost:4173/owner/navigation`
7. 登录后物业端：`http://localhost:4173/admin/monitor`
8. 诊断地址（仅备用）：
...
1. 业主端业务链路
   - 先进入登录页，用业主演示账号进入系统
   - 先展示左侧切换区和首页页头，说明当前版本保留 `首页 / 订单 / 导航` 三段式结构
   - 查看首页摘要卡片、预约参数和推荐结果
   - 创建预约并进入订单页，展示订单状态、费用和结算动作
   - 切到导航页，展示页面内地图预览与外部地图入口

2. 物业端业务链路
   - 返回登录页，用物业演示账号进入系统
   - 查看监管页顶部摘要板，一屏说明占用率、今日收益、活动预约和调度策略
   - 展示实时 / 降级标签、数据来源、更新时间和失败原因
   - 查看收益趋势、区域收益和占用率趋势三组图表
